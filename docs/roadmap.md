...
- `agent-session`: 文件 Session 持久化、恢复判断和最小内存搜索已实现；SQLite/FTS 和跨进程查询待实现。
- `agent-core`: 错误事件化和恢复预算已实现；错误分类、退避和 provider fallback 待实现。

## Phase 4: 记忆与技能

- `agent-memory`: 长期记忆写入条件、重复拦截和容量边界已实现；文件持久化、快照注入和审批待实现。
- `agent-session`: Session Search 最小内存索引已实现；SQLite/FTS、scroll 和 browse 待实现。
- `agent-skill`: Skills 加载、任务匹配、来源记录、信任策略、技能候选生成、内存审批流和 Trajectory 自进化复盘最小链路已实现；文件 pending、diff 审批和人工确认界面待实现。
- `agent-learning`: Memory、Skill、显式关系和可解释关联边的 Learning Graph 已实现；持久化 mutation、回滚和查询接口待实现。

## Phase 5: Cron、Gateway 与中断恢复

- `agent-gateway`: HTTP Gateway 最小入口契约已实现；真实 HTTP Server、认证、SSE、Runs API 和审批回调待实现。
